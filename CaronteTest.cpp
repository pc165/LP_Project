...
bool Test::IgualFitxers(string nomF1, string nomF2, string nomFRes) {
    ifstream f1, f2;
    f1.open(nomF1);
    f2.open(nomF2);
    bool iguals = true;
    ...
    if (f1.is_open() && f2.is_open()) {
        ...
        while (!f1.eof() && !f2.eof()) {
            ...
            if (a != b) {
                ++cont;
                fitxCompara << flush << "Error num " << cont << " a linia " << linia << endl
                            << flush;
                iguals = false;
            }
        }
