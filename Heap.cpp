...
Heap::Heap(const Heap &h) { //constructor de copia
    m_data = h.m_data;
    m_maxEls = h.m_maxEls;
    m_act = h.m_act;
    m_index = h.m_index;
...
std::ostream &Heap::printRec(std::ostream &out, int pos, int n) const {
    if (pos <= m_act) { //Pintem arbre
        for (int i = 0; i < n; i++) {
            out << "|--";
        }
        out << "|-->" << m_data[pos] << endl;
        if (((2 * pos) + 1) <= m_maxEls) { //printem subarbre esquerre
            printRec(out, fillEsq(pos), n + 1);
        }
        if ((2 * pos) < m_maxEls) { //printem subarbre dret
